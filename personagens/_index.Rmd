            ...
            tags$div(class="col mb-4",
                tags$div(class="card h-100",
                    tags$div(class="container-fluid no-padding",
                             style=paste0("background-image: url('", path,"%spersonagens/_index/", city,"_avatar_bg.png')"),
                             ...
                             tags$a(href=sprintf("/personagens/%s", i),
                                tags$div(style="text-align: center;", 
                                         tags$img(src=sprintf("%spersonagens/_index/avatares/%s_avatar_%s.png", path, city, i)
                                                  )
                                         )
                                )
                             ),
...
tagAppendChild(
    ...
    tagList(gera_cod(city = "mon", char_by_city = char_df_tidy %>% filter(city == "mondstadt") %>% select(name) %>% as.matrix())))
```
...
tagAppendChild(
    tags$div(
        class="row row-cols-1 row-cols-md-4"),
    tagList(gera_cod(city = "liy", char_by_city = char_df_tidy %>% filter(city == "liyue") %>% select(name) %>% as.matrix())))
```
...
tagAppendChild(
    tags$div(
        class="row row-cols-1 row-cols-md-4"),
    tagList(gera_cod(city = "sne", char_by_city = char_df_tidy %>% filter(!(city %in% c("mondstadt", "liyue", "paimon"))) %>% select(name) %>% as.matrix())))
```
